// Napredna pretraga - JavaScript modul

// Interface za autocomplete item
class AutocompleteItem {
  constructor(text, type, relevance) {
    this.text = text;
    this.type = type;
...
export class AdvancedSearch {
  ...
  async initializeSearch() {
    try {
      ...
      const { contentScanner } = await import('../utils/contentScanner.js');
  ...
  async performSearch(query) {
    try {
      ...
      // Izvrši pretragu
      const { searchIndexer } = await import('../utils/searchIndexer.js');
  ...
  updateResultsHeader(resultCount, query) {
