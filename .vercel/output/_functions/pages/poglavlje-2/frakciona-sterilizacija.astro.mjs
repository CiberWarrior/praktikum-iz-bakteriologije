/* empty css                                    */
import { c as createAstro, a as createComponent, e as renderComponent, f as renderScript, d as renderTemplate, m as maybeRenderHead } from '../../chunks/astro/server_B-kAR108.mjs';
import 'kleur/colors';
import { $ as $$SubPageLayoutWithBase } from '../../chunks/SubPageLayoutWithBase_DdfsaLU4.mjs';
export { renderers } from '../../renderers.mjs';

const $$Astro = createAstro("https://ciberwarrior.github.io");
const $$FrakcionaSterilizacija = createComponent(($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$Astro, $$props, $$slots);
  Astro2.self = $$FrakcionaSterilizacija;
  const {
    title = "Frakciona sterilizacija (tindalizacija)",
    description = "Postupna sterilizacija za materijale osjetljive na visoku temperaturu"
  } = Astro2.props;
  return renderTemplate`${renderComponent($$result, "SubPageLayoutWithBase", $$SubPageLayoutWithBase, { "chapterNumber": 2, "chapterNumber": 2, "title": title, "description": description, "chapterNumber": 2, "primaryColor": "blue", "breadcrumbPath": [
    { name: "Frakciona sterilizacija (tindalizacija)", url: "/poglavlje-2/frakciona-sterilizacija/" }
  ] }, { "default": ($$result2) => renderTemplate`  ${maybeRenderHead()}<div class="flex max-w-7xl mx-auto"> <!-- Table of Contents Sidebar --> <aside class="hidden lg:block w-64 p-6 bg-gradient-to-b from-gray-50 to-gray-100 border-r border-gray-200 sticky top-0 h-screen overflow-y-auto shadow-lg"> <div class="mb-6"> <div class="flex items-center mb-6"> <div class="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center mr-3"> <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path> </svg> </div> <h3 class="text-xl font-bold text-gray-900">Sadržaj stranice</h3> </div> <!-- Bookmark Button --> <button id="bookmark-btn" class="flex items-center space-x-3 text-sm text-gray-600 hover:text-blue-600 transition-all duration-300 mb-6 p-3 rounded-lg hover:bg-blue-50 w-full"> <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"></path> </svg> <span id="bookmark-text" class="font-medium">Označiti kao bookmark</span> </button> <!-- Table of Contents --> <nav class="space-y-1"> <div class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Sadržaj stranice</div> <a href="#opis-metode" class="toc-link group flex items-center text-sm text-gray-600 hover:text-blue-600 transition-all duration-300 py-2 px-3 rounded-lg hover:bg-blue-50" data-target="opis-metode"> <span class="w-2 h-2 bg-blue-400 rounded-full mr-3 transition-all duration-300"></span>
Opis metode
</a> </nav> </div> </aside> <!-- Main Content --> <main class="flex-1 px-6 py-8"> <div class="max-w-4xl mx-auto"> <div class="prose prose-lg max-w-none"> <!-- Content Blocks --> <div class="space-y-8"> <!-- Opis metode --> <div id="opis-metode" class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 shadow-lg border border-blue-200"> <div class="flex items-center mb-4"> <span class="text-3xl mr-4">🌡️</span> <h3 class="text-2xl font-bold text-blue-900">Opis metode</h3> </div> <div class="bg-white rounded-lg p-6 shadow-sm"> <h4 class="text-xl font-semibold text-purple-800 mb-4">Frakciona sterilizacija (tindalizacija)</h4> <div class="space-y-4"> <p class="text-gray-700 leading-relaxed">
Metodu je uveo John Tyndall, irski fizičar oko 1850. Para koja struji iz uzavrele vode upotrebljava se za sterilizaciju hranjivih medija sa sastojcima koji ne podnose temperaturu iznad 100°C (termolabilne komponente). Aparat koji služi za sterilizaciju u pari naziva se Kochov lonac.
</p> <p class="text-gray-700 leading-relaxed">
To je cilindrična metalna posuda s poklopcem, u kojoj se voda zagrijava električnim grijačima. Iznad vode koja vrije nalazi se rešetka, na koju se stavlja hranjivi medij u Erlenmayerovoj tikvici tako da para struji oko nje. Tikvica s medijem za frakcijsku sterilizaciju stavlja se u Kochov lonac (može se upotrijebiti i obični, veći kuhinjski lonac s poklopcem) i kad se pojavi para te počinje izlaziti, ispod poklopca ili kroz otvor na aparatu počinje se mjeriti vrijeme od 30 min. Nakon toga inkubira se tikvica s podlogom u termostatu na 35°C/24 h. Sljedeća dva dana isti se postupak ponavlja.
</p> <p class="text-gray-700 leading-relaxed">
Cilj je metode da se zagrijavanjem na 100°C unište vegetativni oblici bakterija, a sveprisutni sporogeni oblici otporni na kuhanje preživljavaju. Inkubacijom u hranjivom mediju na povoljnoj temperaturi spore u roku od 24 h prokliju pa se onda vegetativni oblici nastali iz spora ubijaju ponovnim zagrijavanjem na 100°C idućeg dana. Za svaku sigurnost postupak se ponavlja i trećeg dana.
</p> </div> </div> </div> </div> </div> </div> </main> </div>  <div class="flex justify-between mt-16 mb-8 max-w-4xl mx-auto px-4 sm:px-6"> <!-- Prethodna stranica --> <a href="/poglavlje-2/kuhanje/" class="inline-flex items-center justify-center px-8 py-4 bg-gradient-to-r from-gray-600 to-gray-700 text-white font-semibold rounded-full shadow-lg hover:shadow-xl transition-all duration-300 no-underline transform hover:-translate-y-1" aria-label="Prijeđi na prethodnu stranicu: kuhanje">
←
<span class="text-lg">kuhanje</span> </a> <!-- Iduća stranica --> <a href="/poglavlje-2/autoklaviranje/" class="inline-flex items-center justify-center px-8 py-4 bg-gradient-to-r from-emerald-600 to-emerald-700 text-white font-semibold rounded-full shadow-lg hover:shadow-xl transition-all duration-300 no-underline transform hover:-translate-y-1" aria-label="Prijeđi na sljedeću stranicu: autoklaviranje"> <span class="text-lg">autoklaviranje</span>
→
</a> </div> ` })} ${renderScript($$result, "/Users/renchi/Desktop/praktikum-iz-bakteriologije/src/pages/poglavlje-2/frakciona-sterilizacija.astro?astro&type=script&index=0&lang.ts")}`;
}, "/Users/renchi/Desktop/praktikum-iz-bakteriologije/src/pages/poglavlje-2/frakciona-sterilizacija.astro", void 0);

const $$file = "/Users/renchi/Desktop/praktikum-iz-bakteriologije/src/pages/poglavlje-2/frakciona-sterilizacija.astro";
const $$url = "/poglavlje-2/frakciona-sterilizacija";

const _page = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  default: $$FrakcionaSterilizacija,
  file: $$file,
  url: $$url
}, Symbol.toStringTag, { value: 'Module' }));

const page = () => _page;

export { page };
